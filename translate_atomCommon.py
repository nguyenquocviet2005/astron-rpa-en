...
def main():
    ...
    with open("E:/astron-rpa/atomCommon_original.json", "r", encoding="utf-8-sig") as f:
        data = json.load(f)
    ...
    print("Translation complete! Saved to atomCommon_translated.json")
    
    # Also generate the SQL escaped version
    json_str = json.dumps(translated_data, ensure_ascii=False, separators=(',', ':'))
    # Escape for SQL
    json_escaped = json_str.replace("\\", "\\\\").replace("'", "\\'")
    
    # Generate the complete SQL INSERT statement
    sql = f"""-- Replace the atomCommon record with English translations
-- First delete the old record, then insert the new one
DELETE FROM `c_atom_meta` WHERE `atom_key` = 'atomCommon';
INSERT INTO `c_atom_meta` (`id`, `parent_key`, `atom_key`, `atom_content`, `deleted`, `creator_id`, `create_time`, `updater_id`, `update_time`, `version`, `sort`, `version_num`) VALUES
(14, 'root', 'atomCommon', '{json_escaped}', 0, '1', NOW(), 1, NOW(), '1.0.0', NULL, '1000000');
"""
    
    with open("E:/astron-rpa/update_atomCommon.sql", "w", encoding="utf-8") as f:
        f.write(sql)
    
    print("SQL update file saved to update_atomCommon.sql")
